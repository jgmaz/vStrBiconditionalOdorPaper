function R2 = runDecoder(cfg_in,FR,W_int,PCids,R2)
% proj FRs onto dPCs
cfg_def = [];
cfg_def.remCtrl = 0;
...
[W_sort, W_idx] = sort(abs(W_int),1,'descend');
nCells = size(W_int,1);
        ...
        for iW = 1:wSlide:cfg.nBins-wLen
            ...
            for iC = 1:iT
                ...
                switch method
                    ...
                    case{'LSE'}
                        Wts = (train_set(:,1:nPreds)'*train_set(:,1:nPreds))\(train_set(:,1:nPreds)'*train_set(:,resp_idx));
                end
            ...
            R2_shuff_mean{iD}{count_rem}(:,count) = mean(R2_shuff);
            R2_shuff_std{iD}{count_rem}(:,count) = std(R2_shuff);
            
            R2_shuff_comb_mean{iD}{count_rem}(:,count) = mean(R2_shuff_comb);
            R2_shuff_comb_std{iD}{count_rem}(:,count) = std(R2_shuff_comb);
            
            R2_all{iD}{count_rem}(:,count) = R2_temp;
            R2_all_comb{iD}{count_rem}(:,count) = R2_comb;
...
R2.remove.data.trial = R2_all;
R2.remove.data.all = R2_all_comb;
R2.remove.shuff.trial.mean = R2_shuff_mean;
R2.remove.shuff.trial.std = R2_shuff_std;
R2.remove.shuff.all.mean = R2_shuff_comb_mean;
R2.remove.shuff.all.std = R2_shuff_comb_std;
